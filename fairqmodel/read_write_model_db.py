"""Read and write trained model objects and metadata to and from ClickHouse."""
...
def save_model_to_db(
    ...
    model_2_description: str | None,
    execution_time: datetime.datetime,
) -> int:
    """Store a given model and corresponding information into the DB.
    ...
    """
    logger.info("Write model to DB")
    depvar = models.depvar

    # Get new id:
    with db_connect_target() as db:
        model_id = db.query_dataframe(get_query("model_id_max")).to_numpy().item() + 1
...
def retrieve_model_from_db(model_id: int) -> ModelWrapper:
    ...
    model_2 = (
        ...
        if model_object_residuals_str is not None and not pd.isna(model_object_residuals_str)
        else None
    )
...
def write_model_to_models_final(depvar: str, domain: str, model_id: int) -> None:
    """Write the latest model ID to the database as the productive model.
