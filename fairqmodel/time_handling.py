"""Provide timezone-aware time utilities for Berlin-based model scheduling."""
...
FORECAST_HOUR_AFTERNOON = 15
FORECAST_HOUR_MORNING = 5


def get_current_local_time() -> datetime:
    """Return the current date in Berlin time."""
    utc_now = datetime.now(UTC).replace(tzinfo=pytz.utc)
    return utc_now.astimezone(pytz.timezone("Europe/Berlin")).replace(microsecond=0)
...
def get_model_start_time(twice_daily: bool = True) -> pd.Timestamp:
    ...
    if twice_daily:
        if berlin_now.hour >= FORECAST_HOUR_AFTERNOON:
            date_time_forecast = berlin_now.replace(hour=FORECAST_HOUR_AFTERNOON)
        elif berlin_now.hour < FORECAST_HOUR_MORNING:
            previous_day = berlin_now - pd.Timedelta(1, "day")
            date_time_forecast = previous_day.replace(hour=FORECAST_HOUR_AFTERNOON)
        else:  # berlin_now.hour < FORECAST_HOUR_AFTERNOON or berlin_now.hour >= FORECAST_HOUR_MORNING
            date_time_forecast = berlin_now.replace(hour=FORECAST_HOUR_MORNING)
...
def timestamp_to_tz_aware(date: pd.Timestamp) -> pd.Timestamp:
    """Turn a timestamp into a timezone-aware timestamp in Europe/Berlin timezone.
