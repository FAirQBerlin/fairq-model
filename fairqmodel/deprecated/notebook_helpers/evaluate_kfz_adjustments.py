...
def plot_kfz_prediction(
    ...
    for idx_out, j in enumerate(range(0, pred_per_day, 5)):  # Loop over different percentages (only for 0, 50 and 100%)
        ...
        plt.plot(
            x_ticks,
            np.array(pred) + tweak_value,
            label=f"Pred. {j * 10} % kfz",
            color=colors[idx_out],
        )

        max_value = max(max_value, max(pred))
    ...
    plt.title(
        f"Forecast for {depvar} starting at the {date_forecast} for the upcoming {actual_forecast_days} days at station {station_id}"
    )
